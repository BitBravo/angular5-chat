...
class Routes{
	...
	appRoutes(){
		...
		this.app.post('/changePassword',(request,response) =>{console.log(request.body)

			const data = {
				id : (request.body.userid).toLowerCase(),
				password : request.body.password
			};
			...
			if(data.id === '' || data.id === null) {
	            ...
	            response.status(412).json(loginResponse);
	           	...
	           	helper.changePassword( data, (error,result)=>{
					console.log(result)
	           		if (error || result === null ) {
